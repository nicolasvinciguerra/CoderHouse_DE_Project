...
def get_data(base_url, endpoint, headers, params=None):
    ...
    try:
        ...
        try:
            ...
            logging.error("Error en la extraccion de resultados del request. Formato de respuesta no esperado.")
            return None
        return data
...
def connect_to_db (config:ConfigParser, config_section):
    ...
    try:
        if not config.has_section(config_section):
            raise Exception(f"No se encontró la sección {config_section} en el archivo de configuracion.")
    
        # construye sqlalchemy URL
        url = URL.create(
            drivername=config[config_section]["driver"], # indicate driver and dialect will be used
